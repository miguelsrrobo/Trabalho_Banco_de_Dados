def insert_into_table_evaluation(conn, id,  nome, cnpj, endereco, marca):
  cursor = conn.cursor()
...
def select_element_in_table_evaluation(conn):
  cursor = conn.cursor()
...
def deleting_data_in_tables_evaluation(conn, id):
...
def update_in_tables_evaluation(conn, nome_antigo, nome_aser_substituido):
  mycursor = conn.cursor()
